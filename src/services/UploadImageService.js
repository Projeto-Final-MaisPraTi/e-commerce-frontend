...
export const uploadImages = async (images) => {
    ...
    if (!user) {
        console.error('Usuário não está autenticado. Por favor, faça login antes de tentar fazer o upload.');
    ...
    const uploadPromises = images.map((image, index) => {
        return new Promise((resolve, reject) => {
            ...
            const storageRef = ref(storage, `images/image_${index}_${Date.now()}.${contentType.split('/')[1]}`); // Define o nome da imagem no Firebase Storage
